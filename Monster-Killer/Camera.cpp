...
GLvoid CCamera::MoveRotate(GLfloat m_angle, CVector vPoint, CVector vAxis)
{
	...
	CVector vTempEye = vEyeOffset;
	CVector vTempCenter = vCenterOffset;

	vTempEye.SetVector(((GLfloat)pow(vAxis.x, 2) + ((GLfloat)pow(vAxis.y, 2) + (GLfloat)pow(vAxis.z, 2)) * cosTheta) / dotAxis * vTempEye.x +
					   (vAxis.x * vAxis.y * (1 - cosTheta) / dotAxis - vAxis.z * sinTheta / magnitudeAxis)					   * vTempEye.y +
					   (vAxis.x * vAxis.z * (1 - cosTheta) / dotAxis + vAxis.y * sinTheta / magnitudeAxis)					   * vTempEye.z ,
					   (vAxis.y * vAxis.x * (1 - cosTheta) / dotAxis + vAxis.z * sinTheta / magnitudeAxis)					   * vTempEye.x +
					   ((GLfloat)pow(vAxis.y, 2) + ((GLfloat)pow(vAxis.x, 2) + (GLfloat)pow(vAxis.z, 2)) * cosTheta) / dotAxis * vTempEye.y +
					   (vAxis.y * vAxis.z * (1 - cosTheta) / dotAxis - vAxis.x * sinTheta / magnitudeAxis)					   * vTempEye.z ,
					   (vAxis.z * vAxis.x * (1 - cosTheta) / dotAxis - vAxis.y * sinTheta / magnitudeAxis)					   * vTempEye.x +
					   (vAxis.z * vAxis.y * (1 - cosTheta) / dotAxis + vAxis.x * sinTheta / magnitudeAxis)					   * vTempEye.y +
					   ((GLfloat)pow(vAxis.z, 2) + ((GLfloat)pow(vAxis.x, 2) + (GLfloat)pow(vAxis.y, 2)) * cosTheta) / dotAxis * vTempEye.z );

	vTempCenter.SetVector(((GLfloat)pow(vAxis.x, 2) + ((GLfloat)pow(vAxis.y, 2) + (GLfloat)pow(vAxis.z, 2)) * cosTheta) / dotAxis * vTempCenter.x +
						  (vAxis.x * vAxis.y * (1 - cosTheta) / dotAxis - vAxis.z * sinTheta / magnitudeAxis)				 	  * vTempCenter.y +
						  (vAxis.x * vAxis.z * (1 - cosTheta) / dotAxis + vAxis.y * sinTheta / magnitudeAxis)				 	  * vTempCenter.z ,
						  (vAxis.y * vAxis.x * (1 - cosTheta) / dotAxis + vAxis.z * sinTheta / magnitudeAxis)				 	  * vTempCenter.x +
						  ((GLfloat)pow(vAxis.y, 2) + ((GLfloat)pow(vAxis.x, 2) + (GLfloat)pow(vAxis.z, 2)) * cosTheta) / dotAxis * vTempCenter.y +
						  (vAxis.y * vAxis.z * (1 - cosTheta) / dotAxis - vAxis.x * sinTheta / magnitudeAxis)					  * vTempCenter.z ,
						  (vAxis.z * vAxis.x * (1 - cosTheta) / dotAxis - vAxis.y * sinTheta / magnitudeAxis)					  * vTempCenter.x +
						  (vAxis.z * vAxis.y * (1 - cosTheta) / dotAxis + vAxis.x * sinTheta / magnitudeAxis)					  * vTempCenter.y +
						  ((GLfloat)pow(vAxis.z, 2) + ((GLfloat)pow(vAxis.x, 2) + (GLfloat)pow(vAxis.y, 2)) * cosTheta) / dotAxis * vTempCenter.z );

	vEyeOffset = vTempEye;
	vCenterOffset = vTempCenter;
}
